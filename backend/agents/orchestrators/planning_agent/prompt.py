...
- Use the `parallel_travel_search` tool to find flight and hotel choices in parallel (this will search for both flights and hotels simultaneously),
- Use the `train_search_agent` tool to find train choices,
- Use the `search_synthesis_agent` tool to combine and present parallel search results,
- Use the `flight_seat_availability_agent` tool to find seat choices,
...
- You have access to `parallel_travel_search` (which searches for flights and hotels in parallel) and `flight_seat_availability_agent`.
- Given the user's home city location "{origin}" and the derived destination, 
  - Call `parallel_travel_search` which will search for both flights and hotels in parallel. Focus on the flight results.
  ...
  - Call the `flight_seat_availability_agent` tool to show seat options, asks the user to select one.
  - Call the `memorize` tool to store the outbound and inbound flights and seats selections info into the following variables:
    ...
    - For flight choice, store the full JSON entries from the parallel search's flight results.  
...
- You have access to `parallel_travel_search` (which searches for flights and hotels in parallel). For hotel room selection, you'll need to work with the hotel results from the parallel search.
- Given the derived destination and the interested activities,
  - Call `parallel_travel_search` which will search for both flights and hotels in parallel. Focus on the hotel results.
  - Work with the user to select a hotel from the results. When user selects the hotel...
  - Use the `memorize` tool to store the hotel and room selections into the following variables:
    - `hotel_selection` and `room_selection`
    - For hotel choice, store the chosen JSON entry from the parallel search's hotel results.  
...
Remember that you can only use the tools to complete your tasks: 
  - `parallel_travel_search` (searches for flights and hotels in parallel),
  - `train_search_agent`,
  - `search_synthesis_agent`,
  - `flight_seat_availability_agent`,
  - `itinerary_agent`,
  - `memorize`
